...
class ConsoleOnlineBookStore:
    ...
    # ===== 사용자(회원) 메뉴 =====
    # (A) 도서조회/장바구니/주문내역조회/내정보 + (B) 배송조회 통합
    member_menu = ['로그아웃', '도서조회', '장바구니', '주문내역조회', '배송조회', '내정보']
    book_menu = ['돌아가기', '도서상세정보', '장바구니담기']
    cart_menu = ['돌아가기', '책삭제', '주문하기']
    order_menu = ['돌아가기', '상세조회', '주문취소']
    order_detail_menu = ['돌아가기', '배송조회']
    my_info_menu = ['돌아가기', '내정보조회', '비밀번호수정', '회원탈퇴']

    # ===== 관리자 메뉴 =====
    # (A) 도서관리/회원관리/주문목록조회 + (B) 배송관리 통합
    admin_menu = ['로그아웃', '도서관리', '회원관리', '주문목록조회', '배송관리']
    book_manager_menu = ['돌아가기', '도서목록', '도서추가', '도서수정', '도서삭제']
    ...
    def sample_books(self):
        self.bsv.add_book('축구의 이해', '정원재', 10000, 5)
        self.bsv.add_book('농구의 정석', '전민수', 20000, 3)
        self.bsv.add_book('야구의 모든 것', '김철수', 15000, 2)
    ...
    def run_member_menu(self):
        while True:
            ...
            if menu == 0:
                ...
                self.menu_view_my_deliveries()   # (B 통합) 배송조회
            elif menu == 5:
...
# book_menu ==================================================================
    ...
    def run_book_menu(self):
        ...
        while True:
            menu = self.select_menu(ConsoleOnlineBookStore.book_menu, '도서메뉴')
            if menu == 0:
                return
            elif menu == 1:
                self.menu_book_detail()
                self.menu_list_books()
            elif menu == 2:
                self.menu_add_cart()
    ...
    def menu_book_detail(self):
        bookId = input('책번호: ')
        print()
        book = self.bsv.get_book_detail(bookId)
        if book:
            print(book)
        else:
...
# cart_menu ==================================================================
    ...
    def run_cart_menu(self):
        while True:
            self.menu_view_cart()
            menu = self.select_menu(ConsoleOnlineBookStore.cart_menu, '장바구니메뉴')
            if menu == 0:
                return
            elif menu == 1:
...
# order_menu ==================================================================
    ...
    def run_order_menu(self):
        ...
        while True:
            menu = self.select_menu(ConsoleOnlineBookStore.order_menu, '주문내역메뉴')
            if menu == 0:
                return
            elif menu == 1:
    ...
    def run_order_detail_menu(self, orderId):
        while True:
            menu = self.select_menu(ConsoleOnlineBookStore.order_detail_menu, '주문상세메뉴')
            if menu == 0:
                return
            elif menu == 1:
    ...
    def run_admin_menu(self):
        while True:
            ...
            if menu == 0:
                ...
                self.run_book_manager_menu()
            elif menu == 2:
                self.run_member_manager_menu()
            elif menu == 3:
                self.menu_list_all_orders()
            elif menu == 4:
                self.run_delivery_manager_menu()   # (B 통합) 배송관리


# book_manager_menu ==================================================================

    def run_book_manager_menu(self):
        while True:
            menu = self.select_menu(ConsoleOnlineBookStore.book_manager_menu, '도서관리메뉴')
            if menu == 0:
                return
            elif menu == 1:
                self.menu_list_books()
    ...
    def menu_add_book(self):
        ...
        try:
            ...
            return
        print()
        if self.bsv.add_book(title, author, price, stock) == True:
            print('도서가 추가되었습니다')
        else:
    ...
    def menu_edit_book(self):
        ...
        if self.bsv.edit_book(bookId, book) == True:
            print('도서가 수정되었습니다')
        else:
            print('ERROR : 존재하지 않는 도서입니다')
    ...
    def menu_delete_book(self):
        bookId = input('삭제할 책번호: ')
        print()
        if self.bsv.remove_book(bookId) == True:
            print('도서가 삭제되었습니다')
        else:
            print('ERROR : 존재하지 않는 도서입니다')
...
# member_manager_menu ==================================================================
    ...
    def run_member_manager_menu(self):
        while True:
            menu = self.select_menu(ConsoleOnlineBookStore.member_manager_menu, '회원관리메뉴')
            if menu == 0:
                return
            elif menu == 1:
...
# delivery_manager_menu (admin) ==================================================================
    ...
    # (B 통합) 관리자 배송관리 메뉴 - A의 DAO/Service를 활용해 실제 동작하도록 구현
    def run_delivery_manager_menu(self):
        while True:
            menu = self.select_menu(ConsoleOnlineBookStore.delivery_manager_menu, '배송관리메뉴')
            if menu == 0:
                return
            elif menu == 1:
