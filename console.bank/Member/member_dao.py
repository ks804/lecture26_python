...
if __name__ == '__main__':
    dao = MemberDAO()
    print(dao.is_exist('soonbeom'))

    member = Member('soonbeom', '1234', '권순범')
    dao.insert_member(member)
    member = Member('curi', '1234', '1234')
    dao.insert_member(member)
    print(dao.get_member_info('soonbeom'))
    ...
    member = dao.get_member_info('soonbeom')
    if member:
        member.set_pw('1111')
        dao.update_member_info('soonbeom', member)
    
    members = dao.get_all_members()
    for member in members:
        print(member)

    dao.remove_member('soonbeom')
    members = dao.get_all_members()
    for member in members:
        print(member)