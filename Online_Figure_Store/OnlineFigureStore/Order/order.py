class Order:
    ...
    def __str__(self):
        ...
        for order_item in self.__orderItems:
            item_info.append(f'{order_item.get_figure().get_name()} {order_item.get_quantity()}개')
        return f'주문번호 : {self.__orderId} 회원아이디 : {self.__memberId} 상품 : {item_info} 총금액 : {self.__totalPrice} 주소 : {self.__address} 주문상태 : {self.__status}'
